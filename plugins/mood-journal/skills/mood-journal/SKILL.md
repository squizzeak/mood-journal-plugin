...
This is an explicit skill instruction and supported-use boundary, not a host-enforced permission or global session lock. Do not claim the plugin disables the host's agent tools or can detect every other open chat. Cross-device use remains supported sequentially. If another session or external edit has changed records, stop conflicting writes, refresh and reconcile before proceeding. Retain highest-version discovery, source revision checks, idempotent retries and read-back: stale search results and interrupted saves can happen with one agent. Do not require a lock service or multi-agent coordination infrastructure for normal use.
...
- **Handoff:** Use [handoffs.md](references/handoffs.md) for clinician summaries, intermediate previews, or explicit source packets. Before numbered handoffs, inventory the complete series and resolve the highest existing/reserved version; never advance from the first search hit. Keep canonical parent and verified coverage baseline distinct. Incomplete discovery or unresolved lineage permits only a provisional unnumbered draft. A preview does not end an active journal session.
...
5. Update durable current-context or clinician notes only when already authorized and materially supported. For every versioned handoff update, including an incidental weekly update, apply [handoffs.md](references/handoffs.md) and refresh the resolved series before writing; journal-save authorization does not bypass that gate. Re-read before editing; preserve history and dated versions. Apply staged inquiry changes after journal verification.
